  ...
  "relative w-full border px-4 py-3 text-sm [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground [&>svg~*]:pl-7",
  {
    variants: {
      variant: {
        ...
        destructive:
          "border-status-error/30 bg-status-error/5 text-foreground [&>svg]:text-status-error",
        warning:
          "border-status-warning/30 bg-status-warning/5 text-foreground [&>svg]:text-status-warning",
      },
    },
    defaultVariants: {
      variant: "default",
